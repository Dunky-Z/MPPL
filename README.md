...
cd MPPL/samples
# 转换文档
docker run --rm -v $(pwd)/:/mppl riscx/mppl:latest -o mppl-sample.pdf mppl-sample.md
# 转换含 Mermaid 图表的文档
docker run --rm -v $(pwd)/:/mppl riscx/mppl:latest -o mppl-mermaid-sample.pdf mppl-mermaid-sample.md
```

Windows Git Bash 下挂载卷需使用双斜杠路径，例如：

```bash
docker run --rm -v "//c/Develop/MPPL/samples:/mppl" riscx/mppl:latest -o mppl-mermaid-sample.pdf mppl-mermaid-sample.md
```

## 构建并发布 Docker 镜像

版本号记录在仓库根目录 `VERSION` 文件（当前 **2.2.0**：Mermaid 导出、章节编号配置、表格智能列宽与长词断行）。
