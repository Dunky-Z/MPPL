...
% 目录层级（section-numbering: false 时关闭自动编号）
$if(disable-section-numbering)$
...
\usepackage{xtab}
% 允许下划线处断行（标识符如 Require_MacroRepairCode）
...
% longtable 贴齐正文左右边界，避免宽表偏移溢出
\setlength{\LTleft}{0pt}
\setlength{\LTright}{0pt}

% 表格单元格内允许更积极的换行，减少长单词/CJK 溢出
\newcommand{\mppltablesloppy}{%
  \sloppy
